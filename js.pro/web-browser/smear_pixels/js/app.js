...
function smear(canvasCtx, smearNum, x, y, pWidth, pHeight)
{
   // Get the ImageData object that represents the rectangle of pixels to smear
   let pixels = canvasCtx.getImageData(x, y, pWidth, pHeight);
   ...
   //   let output_pixels = canvasCtx.createImageData(pixels);
   ...
   {  // For each row
      ...
      { // For each column
         data[i] = (data[i] + data[i - 4] * m) / smearNum;     // Red pixel component
         data[i + 1] = (data[i + 1] + data[i - 3] * m) / smearNum;   // Green
         data[i + 2] = (data[i + 2] + data[i - 2] * m) / smearNum;   // Blue
         data[i + 3] = (data[i + 3] + data[i - 1] * m) / smearNum;   // Alpha component
      }
   }

   // Now copy the smeared image data back to the same position on the canvas
   canvasCtx.putImageData(pixels, x, y);
}
